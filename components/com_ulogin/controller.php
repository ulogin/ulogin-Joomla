...
class UloginController extends JControllerLegacy {
		...
		try {
			...
			if($user_id > 0) {
				...
				$this->sendMessage(array (
					'title' => $title, 'msg' => $msg, 'networks' => $networks, 'type' => 'success',
				));
			}
			...
			$this->sendMessage(array (
				'title' => "Ошибка при работе с БД.", 'msg' => "Exception: " . $e->getMessage(), 'type' => 'error'
			));
	 ...
	 * @param array $params
	 */
	protected function sendMessage($params = array ( 'type' => '', 'title' => '', 'msg' => '' )) {
		$app = JFactory::getApplication();
		$backurl = base64_decode($app->input->get->get('backurl', '', 'BASE64'));
		$params['type'] = isset($params['type']) ? $params['type'] : '';
		$type = ($params['type'] == 'error' || $params['type'] == 'warning') ? $params['type'] : 'message';
	...
	protected function newUloginAccount($u_user_db = '') {
		...
		if(!$emailExists && !$isUserLogined) {
			...
			$other_u = $this->model->getUloginUserItem(array ( 'user_id' => $user_id, ));
			if($other_u) {
	...
	protected function regUser() {
		...
		if(intval($res) > 0) {
			if(JComponentHelper::isEnabled('com_k2', true)) {
				...
				$file_url = (!empty($u_data['photo_big'])) ? $u_data['photo_big'] : (!empty($u_data['photo']) ? $u_data['photo'] : '');
			...
			$this->sendMessage(array (
				'title' => "", 'msg' => JText::_('COM_USERS_REGISTRATION_COMPLETE_VERIFY'), 'type' => 'warning'
			));

			return false;
		} elseif($res === 'useractivate') {
			$this->sendMessage(array (
				'title' => "", 'msg' => JText::_('COM_USERS_REGISTRATION_COMPLETE_ACTIVATE'), 'type' => 'warning'
			));

			return false;
		} else {
			$this->sendMessage(array (
				'title' => "Ошибка при регистрации.", 'msg' => "Произошла ошибка при регистрации пользователя.",
				'type' => 'error'
			));
	...
	protected function addUloginAccount($user_id) {
		$user = $this->model->addUloginAccount(array (
			'user_id' => $user_id, 'identity' => strval($this->u_data['identity']),
			'network' => $this->u_data['network'],
		));
		if(!$user) {
			$this->sendMessage(array (
				'title' => "Произошла ошибка при авторизации.", 'msg' => "Не удалось записать данные об аккаунте.",
				'type' => 'error'
			));
	...
	protected function loginUser($user_id = 0) {
		...
		if(!$CMSuser) {
			$this->sendMessage(array (
				'title' => '', 'msg' => 'Произошла ошибка при авторизации.', 'type' => 'error',
			));
		...
		if($app->login($credentials, $options) === true) {
			...
			if($this->model->checkComponent('com_k2')) {
				...
				if(empty($row->url) || (empty($row->gender) && isset($u_data['sex'])) || (empty($row->image) && (!empty($u_data['photo_big']) || !empty($u_data['photo'])))
				) {
					if($k2exist) {
						$row->id = $k2id;
					}
			...
			if(count($methods) > 1) {
				...
				if(isset($otpConfig->method) && ($otpConfig->method != 'none')) {
					$this->sendMessage(array (
						'title' => "", 'msg' => "Авторизация невозможна при включенной двухфакторной аутентификации.",
						'type' => 'warning'
					));
			...
			$this->sendMessage(array (
				'title' => "Ошибка при авторизации.", 'msg' => "Не удалось осуществить вход.", 'type' => 'error'
			));
...
//todo createAvatar (для K2)
	...
	protected function checkCurrentUserId($user_id) {
		...
		if($this->isUserLogined) {
			...
			$this->sendMessage(array (
				'title' => '',
				'msg' => 'Данный аккаунт привязан к другому пользователю. ' . '</br>Вы не можете использовать этот аккаунт',
				'type' => 'warring',
			));
	...
	protected function uloginParseRequest() {
		...
		if(!$this->token) {
			$this->sendMessage(array (
				'title' => "Произошла ошибка при авторизации.", 'msg' => "Не был получен токен uLogin.",
				'type' => 'error'
			));
		...
		if(!$s) {
			$this->sendMessage(array (
				...
				'msg' => "Не удалось получить данные о пользователе с помощью токена.", 'type' => 'error'
			));
	...
	protected function checkTokenError() {
		if(!is_array($this->u_data)) {
			$this->sendMessage(array (
				'title' => "Произошла ошибка при авторизации.",
				'msg' => "Данные о пользователе содержат неверный формат.", 'type' => 'error'
			));
		...
		if(isset($this->u_data['error'])) {
			...
			switch($this->u_data['error']) {
				case 'token expired':
					$this->sendMessage(array (
						'title' => "Произошла ошибка при авторизации.",
						'msg' => "<i>ERROR</i>: время жизни токена истекло", 'type' => 'error'
					));
					break;
				case 'invalid token':
					$this->sendMessage(array (
						'title' => "Произошла ошибка при авторизации.", 'msg' => "<i>ERROR</i>: неверный токен",
						'type' => 'error'
					));
					break;
				default:
					$this->sendMessage(array (
						'title' => "Произошла ошибка при авторизации.",
						'msg' => "<i>ERROR</i>: " . $this->u_data['error'], 'type' => 'error'
					));
			}
		...
		if(!isset($this->u_data['email'])) {
			$this->sendMessage(array (
				'title' => "Произошла ошибка при авторизации.",
				'msg' => "В возвращаемых данных отсутствует переменная <b>email</b>", 'type' => 'error'
			));
