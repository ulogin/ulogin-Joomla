...
class UloginModelUlogin extends JModelItem {
	...
	public function checkUserName($username = '') {
		$params = array (
			'conditions' => "username LIKE '{$username}'", 'columns' => 'id',
		);
		$result = self::query('users', $params)->execute();
	...
	public function getUserIdByEmail($email = '') {
		$params = array (
			'conditions' => "email LIKE '{$email}'", 'columns' => 'id', 'limit' => 1,
		);
	...
	public function getUloginUserNetworks($user_id = 0) {
		$params = array (
			'conditions' => "user_id = {$user_id}", 'columns' => 'network',
		);
	...
	public function getK2UserID($u_id = 0) {
		$params = array (
			'conditions' => "userID = {$u_id}", 'columns' => 'id',
		);
	...
	public function checkComponent($component_name) {
		$params = array (
			'conditions' => "name = '{$component_name}'", 'columns' => 'enabled',
		);
	...
	static private function query($table, $params = array ()) {
		...
		$query->select($columns)->from($db->quoteName('#__' . $table));
		if($conditions) {
			if(is_array($conditions) && (bool)count(array_filter(array_keys($conditions), 'is_string'))) {
	...
	static private function insert($table, $data = array ()) {
		$db = JFactory::getDBO();
		$query = $db->getQuery(true);
		$query->insert($db->quoteName('#__' . $table))->columns($db->quoteName(array_keys($data)))->values(implode(',', self::escapeKeys($data)));
		$db->setQuery($query);
	...
	static private function delete($table, $conditions) {
		...
		$query->delete($db->quoteName('#__' . $table))->where($conditions);
		$db->setQuery($query);
